...
const faq: Record<UiLocale, FaqCopy> = {
  'zh-TW': {
...
const privacy: Record<UiLocale, PrivacyCopy> = {
  'zh-TW': {
...
function render(locale: UiLocale): void {
  applyLocale(locale);
  ...
  if (page === 'faq') {
    ...
    copy.items.forEach(([question, answer], index) => {
      ...
      const paragraph = document.createElement('p'); paragraph.textContent = answer;
...
const selector = document.getElementById('ui-locale') as HTMLSelectElement | null;
if (selector) {
  render(selector.value as UiLocale);
  selector.addEventListener('change', () => render(selector.value as UiLocale));
}
