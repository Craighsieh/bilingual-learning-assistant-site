...
const dictionaries = { 'zh-TW': zhTW, 'zh-CN': zhCN, ja, ko };
export function uiText(locale, key, values = {}) {
    return dictionaries[locale][key].replace(/\{(\w+)\}/g, (_, name) => String(values[name] ?? `{${name}}`));
}
export function applyLocale(locale) {
    document.documentElement.lang = locale === 'zh-TW' ? 'zh-Hant' : locale === 'zh-CN' ? 'zh-Hans' : locale;
    for (const element of document.querySelectorAll('[data-i18n]')) {
        const key = element.dataset.i18n;
        if (key && key in dictionaries[locale])
            element.textContent = dictionaries[locale][key];
    }
}
